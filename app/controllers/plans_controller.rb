class PlansController < ApplicationController
  ...
  # GET /plans.json
  def index
    @plans = Plan.includes(:user).order(created_at: :desc).page(params[:page])
  end
  ...
  def create
    ...
    respond_to do |format|
      if @plan.save
        format.html { redirect_to @plan, notice: '投稿しました！' }
  ...
  def update
    respond_to do |format|
      if @plan.update(plan_params)
        format.html { redirect_to @plan, notice: '更新しました！' }
  ...
  private
    ...
    def plan_params
      params.require(:plan).permit(:title, :budget, :time, :tmp, :image)
    end
end